...
class ModifiedImageGenerationThread(threading.Thread):
    ...
    def __init__(self, 
                 ...
                 upsampler = None,
                 display_func = None):
        super(ModifiedImageGenerationThread, self).__init__()
        self.name = name
        self.pipe = get_pipe()
        self.seed = seed
        if AE_Thread is None:
            self.Prompt_Thread = None
        else:
            self.Prompt_Thread = AE_Thread.Prompt_Thread
...
def main():
    ...
    #p_thr.start()      
    thr = ModifiedImageGenerationThread(name = "thr", Prompt_Thread=p_thr)
    thr.start()
